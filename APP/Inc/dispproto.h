 ...
 *     0x05 PARAM_DUMP 全量配置：gRadarConfig 序列化（见下方布局）                            = 81B
 *   上行（显示板 -> 主板）
 ...
 *
 * 说明：本文件只规定帧结构与校验；字段映射在各板 app_disp.c 中实现。
 * ============================================================ */
...
#define DISP_CMD_SET_PARAM     0x84   /* param_id(u8) + value(f32) = 5B */
#define DISP_CMD_SET_STR       0x85   /* param_id(u8) + len(u8) + data = 2B+len */
 ...
 * 与显示板 oled_ui.h 中 RADAR_PARAM 配置字段一一对应。
 * ============================================================ */
typedef enum
{
